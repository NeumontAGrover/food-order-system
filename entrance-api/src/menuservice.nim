...
proc getMenuItem*(req: Request, headers: HttpHeaders, id: string) {.async, gcsafe.} =
    let client = newAsyncHttpClient()
    ...
    try:
        let response = await client.getContent("http://menu-service:21991/menu/" & id)
        await req.respond(Http200, response, headers)
    except:
        await req.respond(Http500, "{\"message\":\"Could not get document\"}", headers)
